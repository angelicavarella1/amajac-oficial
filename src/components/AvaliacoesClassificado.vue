=== C:\Users\angel\Documents\Projetos\amajac-oficial\src\components\AvaliacoesClassificado.vue ===
<template>
      ...
      >
        <div class="flex items-start gap-4">
          <div class="flex-shrink-0">
            <div class="image-wrapper relative w-12 h-12">
              <template v-if="avaliacao._userImage.loading">
                <div class="w-12 h-12 rounded-full bg-gray-300 dark:bg-gray-600 flex items-center justify-center animate-pulse">
                  <span class="text-xs text-gray-500">...</span>
                </div>
              </template>
              <template v-else-if="avaliacao._userImage.error || !avaliacao._userImage.imageUrl">
                <div class="w-12 h-12 rounded-full bg-gray-200 dark:bg-gray-600 flex items-center justify-center border-2 border-red-300 dark:border-red-700">
...
function formatDate(dateString) {
  // Garante que a data seja formatada, adicionando uma proteção caso a data seja inválida
  try {
...
// Lifecycle
onMounted(() => {
  console.log('🛠️ Iniciando sistema de avaliações para:', props.classificadoId);
