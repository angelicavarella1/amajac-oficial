      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-8 mb-6">
        ...
        <div class="md:col-span-1">
          ...
          <ul class="space-y-2">
            ...
            <li><router-link to="/noticias" class="hover:underline">Notícias</router-link></li>
            ...
            <li><router-link to="/politica" class="hover:underline">Política de Privacidade</router-link></li>
          </ul>
        </div>
        ...
        <div class="md:col-span-1">
          ...
          <div class="mt-4 p-3 bg-amajac-600 rounded-lg">
            <h4 class="text-sm font-semibold mb-1">Horário de Atendimento</h4>
...
const handleLogoError = (event) => {
  ...
  // Fallback: tentar caminho alternativo após um tempo
  setTimeout(() => {
    if (logoError.value) {
...
onMounted(async () => {
  console.log("FooterComponent: Carregando configurações...");
  try {
    await configuracoesStore.carregarConfiguracoes();
    console.log("FooterComponent: Configurações carregadas com sucesso.");
  } catch (error) {
    console.error("FooterComponent: Erro ao carregar configurações para o rodapé:", error);
  }
});
</script>

<style scoped>
/* Estilos específicos do footer se necessário */
</style>